...
class GPUMonitor:
    ...
    def __init__(self, gpu_index: int = 0):
        ...
        if PYNVML_AVAILABLE:
            try:
                ...
                self.available = True
                print(
                    f"GPU Monitor initialized: Logical GPU {gpu_index} -> Physical GPU {self.physical_gpu_index}"
                )
    ...
    def _get_physical_gpu_index(self, logical_index: int) -> int:
        ...
        visible_devices = [
            int(x.strip()) for x in cuda_visible.split(",") if x.strip().isdigit()
        ]
    ...
    def _print_gpu_info(self):
        ...
        try:
            ...
            print(f"GPU: {name}")
            print(
                f"Memory: {used_gb:.2f}GB used / {total_gb:.2f}GB total ({free_gb:.2f}GB free)"
            )
        except Exception as e:
...
class ExperimentRunner:
    ...
    def __init__(
        ...
        if not self.gpu_monitor.available:
            ...
            print(
                f"Will run maximum {max_parallel_without_monitoring} experiment(s) at a time."
            )
    ...
    def create_log_path(self, exp_config: Dict) -> Tuple[str, str]:
        ...
        category = exp_config.get("category", "unknown")
        ...
        exclude_keys = {
            "category",
            "dataframe_path",
            "dataframe_path_original",
            "save_dir",
            "device",
            "gemini_logo_mask_path",
            "model_path",
        }
        ...
        for key, value in sorted(exp_config.items()):
            if key not in exclude_keys:
                ...
                short_key = (
                    key.replace("model_type", "mt")
                    .replace("backbone", "bb")
                    .replace("lambda_", "lam")
                    .replace("n_per_type", "npt")
                    .replace("seeds", "s")
                )
    ...
    def start_experiment(
    ...
    ) -> Optional[Tuple[subprocess.Popen, Any]]:
        """
        ...
        print(f"  Category: {exp_config.get('category', 'N/A')}")
        ...
        while not self.experiment_queue.empty() or self.running_processes:
            ...
            if not self.experiment_queue.empty():
                ...
                if self.gpu_monitor.available:
                    ...
                    if not can_start:
                        print(
                            f"Waiting for more GPU memory (need {self.min_free_memory_percent}% free)..."
                        )
                else:
                    # Fallback: limit based on number of running experiments
                    can_start = (
                        len(self.running_processes)
                        < self.max_parallel_without_monitoring
                    )
                    if not can_start:
                        print(
                            f"Waiting for running experiments to finish (limit: {self.max_parallel_without_monitoring})..."
                        )
                ...
                if can_start:
                    ...
                    if result:
                        ...
                        log_info = f"Category: {exp_config.get('category', 'N/A')}, seeds: {exp_config.get('seeds', 'N/A')}"
...
def main():
    ...
    hyperparameters = {
        ...
        "dataframe_path": ["cbm_vad/cbm_data/mvtec/{category}_dataset_automated.csv"],
        # 'dataframe_path_original': ['cbm_data/mvtec/{category}_dataset_automated_gen_concepts.csv'],
        "save_dir": [
            "cbm_vad/cbm_models/mvtec_few_real_anomalies/{category}_models/cont3/seeds_{seeds}"
        ],
        "model_path": ["cbm_vad/cbm_models/backbones/fine-tuned-mobilenet.pth"],
        "gemini_logo_mask_path": ["datasets/gemini_logo_mask.png"],
        # Boolean flags
        "use_concepts": [True],
        "use_gen_anomalies": [False],
        "contaminate": [False],
        "subsample_anomalies": [True],
    }
    ...
    for exp in experiments:
        exp_copy = exp.copy()
        category = exp_copy.get("category", "")
        seeds = exp_copy.get("seeds", 0)

        # Replace {category} and {seeds} placeholders in paths
        for key in ["dataframe_path", "dataframe_path_original", "save_dir"]:
            if key in exp_copy and isinstance(exp_copy[key], str):
                exp_copy[key] = exp_copy[key].format(category=category, seeds=seeds)
