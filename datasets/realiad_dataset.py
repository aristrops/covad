...
class RealIadDataset(IadDataset):
    ...
    #extract the view from the filename
    def extract_view(self, filename: str) -> Optional[str]:
    ...
    #select view with most anomalous images
    def select_best_view(self, entries):
        view_counts = {}
        ...
        print(f"Keeping only view {best_view} with {view_counts[best_view]} anomalous images")
    ...
    def load_dataset(self):
        ...
        for image_path in root.rglob("*.jpg"):
            ...
            view = self.extract_view(str(image_path))
        ...
        best_view = self.select_best_view(image_entries)
        
        #keep only entries of the selected view
        image_entries = [e for e in image_entries if e["view"] == best_view]
